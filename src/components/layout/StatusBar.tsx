...
import { Zap, Clock, Terminal, Sparkles } from 'lucide-react';
  ...
  return (
    <>
      <footer className="h-6 bg-neutral-900 border-t border-neutral-800 px-3 flex items-center justify-between text-[11px] text-neutral-400 select-none shrink-0 font-mono">
        ...
        {/* Right items: Chunks, Encoding, and In-App Update Trigger */}
        <div className="flex items-center space-x-3">
          {totalChunks > 0 && (
            <span>
      ...
      <UpdateDialog isOpen={isUpdateOpen} onClose={() => setIsUpdateOpen(false)} />
    </>
  );
};
